...
class ConnectFourBoard(object):
    ...
    def __init__(self, board_array = None, board_already_won = None, modified_column = None, current_player = 1, previous_move = -1, num_to_win=4, longest_chain_wins=False):
        ...
        self.longest_chain_wins = longest_chain_wins
    ...
    def do_move(self, column):
        ...
        return ConnectFourBoard(new_board, board_already_won=self.is_win(), modified_column=column, current_player = self.get_other_player_id(), num_to_win=self.num_to_win, longest_chain_wins=self.longest_chain_wins)
    ...
    def _is_win_from_cell(self, row, col):
        ...
        return self._max_length_from_cell(row, col) >= self.num_to_win
    ...
    def is_win(self):
        ...
        #    return self._is_win
        #else:
        if(self.longest_chain_wins):
            return self.is_win_twenty_moves()
        else:
            for i in xrange(self.board_height):
                for j in xrange(self.board_width):
    ...
    def is_win_twenty_moves(self):
        """
        Return the id# of the player who has won this game.
        Return 0 if it has not yet been won.
        """
        if self.get_total_moves_played() == 20:
            longest_chain_diff = self.get_longest_chain_diff()
            if longest_chain_diff > 0:
                self._is_win = True
                return self.get_current_player_id()
            elif longest_chain_diff < 0:
                self._is_win = True
    ...
    def get_total_moves_played(self):
        countMoves=0
        for i in xrange(self.board_height):
            for j in xrange(self.board_width):
                cell_player = self.get_cell(i,j)
    ...
    def get_longest_chain_diff(self):
        curr_player = self.get_current_player_id()
        longest_chain_curr = self.longest_chain(curr_player)
        other_player = self.get_other_player_id()
        longest_chain_other = self.longest_chain(other_player)

        return longest_chain_curr - longest_chain_other


    def is_game_over(self):
        """ Return True if the game has been won, False otherwise """
        if self.longest_chain_wins:
            return self.is_game_over_twenty_moves()
        else:
            return ( self.is_win() != 0 or self.is_tie() )

    def is_game_over_twenty_moves(self):
        """ Return True if the game has been won, False otherwise """
        return ( self.is_win_twenty_moves() != 0 or self.is_tie_twenty_moves() )

    def is_tie(self):
        """ Return true iff the game has reached a stalemate """
        if self.longest_chain_wins:
            return self.is_tie_twenty_moves()
        else:
            return not 0 in self._board_array[0]

    def is_tie_twenty_moves(self):
        """ Return true iff the game has reached a stalemate """
        if self.get_total_moves_played() == 20:
            longest_chain_diff = self.get_longest_chain_diff()
            if(longest_chain_diff == 0):
    ...
    def clone(self):
        ...
        return ConnectFourBoard(self._board_array, board_already_won=self._is_win, current_player = self.get_current_player_id(), num_to_win=self.num_to_win, longest_chain_wins= self.longest_chain_wins)
...
class ConnectFourRunner(object):
    ...
    * The game ends when one of the two players has four consecutive tokens in a row
    ...
    def __init__(self, player1_callback, player2_callback, board = None, time_limit = 10, num_to_win=4, longest_chain_wins=False):
        ...
        self.longest_chain_wins = longest_chain_wins
        if(board is None):
            self._board = ConnectFourBoard(num_to_win=num_to_win, longest_chain_wins=longest_chain_wins)
        else:
            self._board = board
...
def run_game(player1, player2, board=None, num_to_win=4, longest_chain_wins=False):
    """ Run a game of Connect Four, with the two specified players """
    if(board is None):
        board = ConnectFourBoard(num_to_win=num_to_win, longest_chain_wins=longest_chain_wins)
    game = ConnectFourRunner(player1, player2, board=board, num_to_win=num_to_win, longest_chain_wins=longest_chain_wins)
